...
def test_pluck_invalid():
    ...
    assert (result == expected), assert_ex(
        'pluck error check', result, KeyError)
...
def test_sort_by_valid_asc():
    ...
    assert (result == expected), assert_ex(
        'sort by key asc', result, expected, hint=TEST_KEY)
...
def test_sort_by_valid_desc():
    ...
    assert (result == expected), assert_ex(
        'sort by key desc', result, expected, hint=TEST_KEY)
...
def test_sort_by_invalid():
    ...
    assert (result == expected), assert_ex(
        'sort error check', result, KeyError, hint=invalid)
